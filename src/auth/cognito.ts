...
// /* ---------------------------------------------------------------
//   PHONE VERIFICATION — Triggered post email setup.
// --------------------------------------------------------------- */

// export const sendPhoneVerificationCode = (): Promise<void> => {
//   return new Promise((resolve, reject) => {
//     const cognitoUser = userPool.getCurrentUser();
//     if (!cognitoUser) {
//       return reject(new Error("No active user session to send mobile OTP."));
//     }
    
//     cognitoUser.getSession((err: Error | null, session: CognitoUserSession | null) => {
//       if (err || !session?.isValid()) {
//         return reject(new Error("Session invalid. Please log in to verify your phone."));
//       }
//       cognitoUser.getAttributeVerificationCode('phone_number', {
//         onSuccess: () => resolve(),
//         onFailure: (err) => reject(new Error(friendlyCognitoMessage(err, "Failed to send mobile OTP. Please try again."))),
//       });
//     });
//   });
// };

// export const confirmPhoneVerificationCode = (code: string): Promise<void> => {
//   return new Promise((resolve, reject) => {
//     const cognitoUser = userPool.getCurrentUser();
//     if (!cognitoUser) {
//       return reject(new Error("No active user session to verify mobile OTP."));
//     }
...
//         onSuccess: () => resolve(),
//         onFailure: (err) => reject(new Error(friendlyCognitoMessage(err, "Failed to verify mobile OTP. Please try again."))),
//       });
//     });
//   });
// };
...
/* ---------------------------------------------------------------
  FORGOT PASSWORD / RESET FLOW — Shared for login page
--------------------------------------------------------------- */

/** Step 1: ask Cognito to email a verification code so the candidate can set a new password. */
export const triggerSetPassword = (email: string): Promise<unknown> => {
  return new Promise((resolve, reject) => {
    const cognitoUser = new CognitoUser({ Username: email, Pool: userPool });
    cognitoUser.forgotPassword({
      onSuccess: (data) => resolve(data),
      onFailure: (err) =>
        reject(new Error(friendlyCognitoMessage(err, "Could not send the password reset code. Please try again."))),
      // Some pool configs call this instead of onSuccess once the code is sent.
      inputVerificationCode: (data) => resolve(data),
    });
  });
...
/** Step 2: candidate submits the emailed code + their newly chosen password. */
export const confirmSetPassword = (
  email: string,
  code: string,
  newPassword: string
