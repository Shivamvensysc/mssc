...
/* ---------------------------------------------------------------
   ...
   NOTE: The printed advertisement text says "Dated: 20/07/2026".
   Per explicit instruction this constant is currently set to
   22/07/2026 instead — confirm this against the final/amended
   notification before going live, since the reference date
   directly shifts who passes/fails the age check.
--------------------------------------------------------------- */

// Fixed reference date the app treats as the notification date.
// Age is computed AS ON this date, not the current date.
// months are 0-indexed → 0=Jan ... 6=Jul, 7=Aug
export const NOTIFICATION_DATE = new Date(2026, 6, 22); // 22 July 2026
...
/**
 * Calculates a candidate's age in completed years as on a given
 * reference date (defaults to NOTIFICATION_DATE). Used for the
 * MIN_AGE (18) check and for the live inline "your age is X" style
 * hints — completed-years is the correct convention there.
 */
export const calculateAgeAsOfNotification = (
  dobDay: string,
  dobMonth: string,
...
/**
 * Strict day-level check for the upper age limit. Returns true only
 * if the candidate is MORE than `maxYears` old as on `referenceDate`
 * — i.e. even a single day past their Nth birthday counts as
 * exceeding the limit. Unlike a completed-years comparison, this
 * gives NO grace period into the following year.
 *
 * Example: maxYears = 38, DOB = 19/07/1988, referenceDate = 22/07/2026
 *   cutoffDate (38th birthday) = 19/07/2026
 *   22/07/2026 > 19/07/2026 → true → exceeded, NOT eligible.
 */
export const hasExceededAgeLimit = (
  dobDay: string,
  dobMonth: string,
  dobYear: string,
  maxYears: number,
  ...
  const monthIndex = months.indexOf(dobMonth);
  const year = parseInt(dobYear, 10);
  if (monthIndex === -1 || !day || !year) return false;

  // Exact calendar date the candidate turns `maxYears` years old.
  const cutoffDate = new Date(year + maxYears, monthIndex, day);
...
export const registrationSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    // ---- Date of birth / Age rules ----
    // Per advertisement clause 3(iv): age must be between 18 and 38
    // (plus applicable relaxation) AS ON the date of notification
    // (currently set to 22/07/2026 — see NOTIFICATION_DATE above),
    ...
    if (dob > NOTIFICATION_DATE) {
      ctx.addIssue({
        path: ["dobYear"],
        code: z.ZodIssueCode.custom,
        message: "Date of birth cannot be after the date of notification (22-07-2026)",
      });
      return;
    }
    ...
    if (age < MIN_AGE) {
      ctx.addIssue({
        path: ["dobYear"],
        code: z.ZodIssueCode.custom,
        message: `Candidate must be at least ${MIN_AGE} years old as on the date of notification (22-07-2026)`,
      });
    } else if (hasExceededAgeLimit(data.dobDay, data.dobMonth, data.dobYear, maxAgeLimit)) {
      // Strict day-level check: even 1 day past the max-age birthday
      // is disqualifying — no grace period into the following year.
      ctx.addIssue({
        path: ["dobYear"],
        code: z.ZodIssueCode.custom,
        message:
          maxAgeLimit > BASE_MAX_AGE
            ? `Age cannot exceed ${maxAgeLimit} years (including applicable relaxation) as on the date of notification (22-07-2026)`
            : `Age cannot exceed ${BASE_MAX_AGE} years as on the date of notification (22-07-2026)`,
      });
    }
  });
