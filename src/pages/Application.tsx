...
//     const payment = {
//       mode: paymentMode || '',
//       acknowledged: paymentAcknowledged,
//       amount: 135,
//     };

//     return { personalDetails, document, education, experience, payment };
//   };

//   const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
//     e.preventDefault();

//     // Guard: the actual submission only happens from the Review step. This
//     // stops the form from being submitted early (e.g. pressing Enter inside
//     // a text field while still on Step 1 or Step 2).
//     if (currentStep !== 3) return;

//     setIsSubmitting(true);

//     try {
//       const { personalDetails, document, education, experience, payment } = preparePayload();
...
//                     className="primary-gradient text-white px-16 py-4 rounded-full font-label-md text-[14px] shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center gap-2"
//                   >
//                     Next: Payment
...
//                         </span>
//                         <div>
//                           <p className="text-2xl font-bold text-green-800 leading-none mb-1">₹135</p>
...
//                         After you review and submit your application, you will be redirected to the official payment
//                         gateway. Once payment succeeds, your status updates to "Fee Paid" and a receipt is generated. ·
//                         आपको भुगतान गेटवे पर पुनर्निर्देशित किया जाएगा।
//                       </p>
//                     </div>
...
//                     type="button"
//                     onClick={() => goToStep(3)}
//                     disabled={!paymentMode || !paymentAcknowledged}
...
//                       <span className="font-bold text-green-700">₹135</span>
//                     </div>
//                   </div>
//                 </section>
...
//                 </div>
//                 <h2 className="text-2xl font-bold text-on-surface mb-2">Application Submitted Successfully!</h2>
...
export default function ApplicationForm() {
  ...
  const handleInitiatePayment = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const { paymentOrderId, paymentUrl } = response.data.data;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (paymentStatus === 'completed') {
      toast.info('Payment already completed. Please wait for confirmation.');
      return;
    }
    ...
    // Initiate payment from review step
              ...
              <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
                ...
                <div className="flex items-center justify-center gap-4 pt-8 border-t border-outline-variant/30">
                  ...
                  <button
                    ...
                    className="primary-gradient text-white px-16 py-4 rounded-full font-label-md text-[14px] shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? 'Processing...' : isPaymentInitiated ? 'Redirecting...' : 'Pay & Submit'}
                    <span className="material-symbols-outlined text-[18px]">
                      {isSubmitting || isPaymentInitiated ? 'hourglass_top' : 'payment'}
                    </span>
                  </button>
                </div>
                <p className="text-on-surface-variant font-label-sm text-[12px] text-center">
