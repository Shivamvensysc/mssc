...
// DESIGN TOKENS
// ==========================================
// Navy + muted gold: a deliberate departure from the generic
// green-accent / Tailwind-default look, tuned for an official,
// trustworthy government-application feel.
const theme = {
  navy: '#1E3A5F',
  navyDark: '#132A47',
  navyLight: '#2C4E7A',
...
function SuccessModal({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: 'rgba(19, 42, 71, 0.55)' }}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ...
        style={{ backgroundColor: theme.surface, boxShadow: '0 24px 60px -12px rgba(19,42,71,0.4)' }}
      >
...
/**
 * Outlined "cutout" label input — the label sits notched into the top
 * border, always visible, Material-outlined style. Focus lifts the
 * border to navy with a soft gold glow; icons hint at the field's
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="relative">
        <label
          htmlFor={id}
          className="absolute -top-2 left-3 px-1.5 text-[11px] font-semibold bg-white z-10 tracking-wide"
          style={{ color: error ? theme.error : focused ? theme.navy : theme.textMuted }}
        >
        ...
        <input
          ...
          required={required}
          className="w-full rounded-xl border-2 bg-white py-3 text-sm outline-none transition-all duration-150"
          style={{
            borderColor,
...
/** Outlined "cutout" label select, matching FormField's visual language */
function FormSelect({
  label,
  value,
  onChange,
 ...
 * of the native browser date picker. Auto-advances focus as each
 * segment fills, and composes/decomposes an ISO "YYYY-MM-DD" string
 * so the rest of the form (and the API payload) is unaffected.
 */
function DateField({
  label,
  value,
...
function FileUploadField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-xs font-semibold" style={{ color: theme.textMuted }}>
        {label} {required && <span style={{ color: theme.gold }}>*</span>}
      </label>
      <div
...
function Step1Application({
  ...
  const reservationOptions = ['General', 'OBC', 'SC', 'ST', 'EWS', 'Other'];
            ...
            {data.experience.map((exp, idx) => (
              <div key={idx} className="p-3.5 rounded-lg bg-white" style={{ border: `1px solid ${theme.border}` }}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    <span className="text-[11px] font-medium" style={{ color: theme.textMuted }}>Designation</span>
                    <p className="text-sm font-medium" style={{ color: theme.textPrimary }}>{exp.designation || '—'}</p>
                  </div>
                  <div>
                    <span className="text-[11px] font-medium" style={{ color: theme.textMuted }}>Duration</span>
                    <p className="text-sm font-medium" style={{ color: theme.textPrimary }}>{exp.duration || '—'}</p>
                  </div>
                  <div>
                    <span className="text-[11px] font-medium" style={{ color: theme.textMuted }}>Reason for Leaving</span>
                    <p className="text-sm font-medium" style={{ color: theme.textPrimary }}>{exp.reasonLeaving || '—'}</p>
                  </div>
                  <div>
                    <span className="text-[11px] font-medium" style={{ color: theme.textMuted }}>Certificate</span>
                    <p
                      className="text-sm font-medium flex items-center gap-1"
