...
const step1ValidationSchema = z.object({
  ...
  education: z.object({
    ...
    graduation: educationLevelValidationSchema('Graduation', false),
...
const FIELD_LABELS: Record<string, string> = {
  ...
  rciNumber: 'RCI CRR Number',
   dedCert: 'D.El.Ed / D.Ed Certificate',
...
export default function MultiStepForm() {
  ...
  const submitStep2 = async (applicationId: string) => {
    ...
    if (formData.teacherEligibility.tet1Cert) {
      formDataToSend.append('tet1Cert', formData.teacherEligibility.tet1Cert);
    }
  ...
  const handleNext = async () => {
    if (step === 1) {
     ...
     if (formData.personalInfo.stateGovEmployee === 'yes') {
      ...
      // ADD THIS: Make Reservation Certificate mandatory for SC, ST, OBC(M), OBC(MP)
      const resCategory = formData.personalInfo.reservationCategory?.toLowerCase().replace(/\s+/g, '');
      if (['sc', 'st', 'obc(m)', 'obc(mp)'].includes(resCategory)) {
        requiredDocs.push('reservationCert');
      }
      ...
      const requiredTeacherDocs = ['tet1Cert', 'dedCert'];
      const missingTeacherDocs = requiredTeacherDocs.filter((docKey) => {
        const hasNewFile = !!formData.teacherEligibility[docKey as keyof typeof formData.teacherEligibility];
        const hasPreviousFile = !!uploadedDocuments[docKey];
        return !hasNewFile && !hasPreviousFile;
      });

      const allMissingDocs = [...missingDocs, ...missingTeacherDocs];
      
    if (allMissingDocs.length > 0) {
        const missingLabels = allMissingDocs.map((key) => FIELD_LABELS[key] || key).join(', ');
        const errorMap: Record<string, boolean> = {};
        allMissingDocs.forEach((key) => {
          errorMap[key] = true;
        });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <FormSelect
        ...
        required
        disabled={disabled || stateDisabled || loadingStates.states}
...
const [disabilityOptions, setDisabilityOptions] = useState<Array<{ value: string; label: string }>>([]); // NEW
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (statesResponse.success) {
          ...
          // Force lock Manipur for Permanent Address
          if (map['Manipur']) {
            setData(prev => ({
              ...prev,
              address: {
                ...prev.address,
                permanent: { ...prev.address.permanent, state: 'Manipur' }
              }
            }));
            handleStateChange('Manipur', map);
          }
        }
  ...
  // Ensure Permanent state stays 'Manipur' if it gets modified by draft fetches
  useEffect(() => {
    if (data.address.permanent.state !== 'Manipur' && statesMap['Manipur']) {
      updateField('address', 'state', 'Manipur', 'permanent');
    }
  }, [data.address.permanent.state, statesMap]);
...
<FormSelect 
  ...
  options={yesNoOptions.map(y => ({ value: y, label: y }))}
  required 
  disabled={isDisabled('stateGovEmployee')}
/>
          ...
          <table className="w-full text-left border-collapse min-w-[640px]">
            ...
            <tbody>
              {(['10th', '12th', 'graduation', 'postGraduation'] as const).map((level, idx) => {
                const isMandatory = ['10th', '12th'].includes(level);
...
// ADD THIS: Apply required status for the UI fields
const resCategory = data.personalInfo.reservationCategory?.toLowerCase().replace(/\s+/g, '');
if (['sc', 'st', 'obc(m)', 'obc(mp)'].includes(resCategory)) {
  requiredDocs.push('reservationCert');
}
  ...
  const getDocumentEntries = () => {
   ...
   for (const [key, value] of entries) {
      if (key === 'reservationCert') {
        const category = data.personalInfo.reservationCategory?.toLowerCase().replace(/\s+/g, '');
        // Only show if it matches the mandatory categories
        if (!['sc', 'st', 'obc(m)', 'obc(mp)'].includes(category)) {
          continue;
        }
      }
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {(() => {
            ...
            return Object.entries(allReviewDocs).map(([key, val]) => {
              ...
              if (key === 'reservationCert') {
                const category = data.personalInfo.reservationCategory?.toLowerCase().replace(/\s+/g, '');
                if (!['sc', 'st', 'obc(m)', 'obc(mp)'].includes(category)) return null;
              }
