...
export default function Dashboard() {
  ...
  // Pagination state (Load from sessionStorage)
  const [pageNo, setPageNo] = useState(() => Number(sessionStorage.getItem('dash_page')) || 1);
  const pageSize = 20; // Hardcoded to exactly 20 data records per page

  // Search State (Load from sessionStorage)
  const [searchTerm, setSearchTerm] = useState(() => sessionStorage.getItem('dash_search') || "");
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState(() => sessionStorage.getItem('dash_search') || "");

  // Export State (Load from sessionStorage)
  const [exportFilter, setExportFilter] = useState(() => sessionStorage.getItem('dash_filter') || "all");
  const [isExporting, setIsExporting] = useState(false);

  // Sorting State (Load from sessionStorage)
  const [sortConfig, setSortConfig] = useState<{ key: keyof TableCandidate; direction: SortDirection } | null>(() => {
    const saved = sessionStorage.getItem('dash_sort');
    return saved ? JSON.parse(saved) : null;
  });

  // Save states to sessionStorage whenever they change
  useEffect(() => {
    sessionStorage.setItem('dash_page', pageNo.toString());
    sessionStorage.setItem('dash_search', searchTerm);
    sessionStorage.setItem('dash_filter', exportFilter);
    if (sortConfig) {
      sessionStorage.setItem('dash_sort', JSON.stringify(sortConfig));
    }
  }, [pageNo, searchTerm, exportFilter, sortConfig]);
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      // Only reset to page 1 if the component has already mounted 
      // (meaning the user is actually typing a new search)
