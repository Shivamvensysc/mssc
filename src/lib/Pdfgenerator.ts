 ...
 * ( { success, data } -> pass `data` in here, not the whole envelope ).
 * ============================================================ */
...
/** step2 is a flat bag of document-key -> signed URL (or null), plus applicationId */
...
export interface PDFOptions {
  /** Set false to get the jsPDF instance back without triggering a browser download. */
  save?: boolean;
...
// Change this if your file lives somewhere else / has a different name.
...
/* Document key -> friendly label (mirrors DOC_LABELS in ShowallDeatilsPage.tsx) */
...
// These three are shown in the photo panel, not in the documents list.
...
async function urlToDataURL(
  ...
  try {
    ...
    // Only raster images can be embedded with addImage; PDFs, etc. are skipped
    // (they're still listed + linked in the documents section).
...
function shapeSlipData(apiData: ApiData): SlipData {
  ...
  const correspondenceAddress: AddressData =
    step1.address?.correspondence || step0.address?.correspondence || {};
  ...
  return {
    ...
    hasExperience: !!step1.experience?.hasExperience,
    employerDesignation: dash(step1.experience?.employerDesignation),
    servicePeriodMonths: step1.experience?.servicePeriodMonths
      ? `${step1.experience.servicePeriodMonths} MONTHS`
      : "-",
    reasonForLeaving: dash(step1.experience?.reasonForLeaving),
...
function sectionHeader(pdf: jsPDF, y: number, title: string): number {
  ...
  pdf.setFillColor(27, 94, 63); // theme.navy (#1B5E3F)
...
/** 3-slot photo panel: Live Photo, Photograph, Signature (top-right of the personal details block). */
...
function educationTable(
  ...
  const widths = [26, 62, 52, 20, 30]; // sums to 190 = CONTENT_W
...
/**
 * Lists every uploaded certificate/document as a row. Rows link out to the
 * signed URL via a real PDF link annotation (pdf.textWithLink) so the
 * generated slip stays useful even though PDFs/other non-image documents
 * can't be embedded as pictures the way the photo/signature/live-photo can.
 */
function documentsSection(pdf: jsPDF, y: number, documents: DocRow[]): number {
  ...
  for (const doc of documents) {
    ...
    // pdf.setFillColor(uploaded ? 231, 246, 238 : 253, 238, 236);
    pdf.rect(statusX, y, statusW, rowH, "F");
    pdf.rect(statusX, y, statusW, rowH);
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(6.6);
    if (uploaded && doc.url) {
      pdf.setTextColor(27, 94, 63);
 ...
 * MAIN EXPORT
 * ============================================================
 * Usage (e.g. in ShowallDeatilsPage.tsx's handleDownload):
 *
 *   import { generateApplicationPDF } from "./pdfGenerator";
 *   await generateApplicationPDF(applicationData);
 *
 * `applicationData` is the same prop already used by that page — the
 * unwrapped `data` object from the `/application/steps/all` response.
 * ============================================================ */
...
export async function generateApplicationPDF(
  ...
  // Employment / identity block (full-width wide labels)
  ...
  if (data.hasExperience) {
    y = labelValueRow(pdf, y, "DESIGNATION :", data.employerDesignation, {
      labelW: WIDE_LABEL_W,
    });
    y = labelValueRow(pdf, y, "SERVICE PERIOD :", data.servicePeriodMonths, {
      labelW: WIDE_LABEL_W,
    });
    y = labelValueRow(pdf, y, "REASON FOR LEAVING :", data.reasonForLeaving, {
      labelW: WIDE_LABEL_W,
    });
  } else {
    const rowH = 7;
